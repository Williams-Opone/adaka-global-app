...
export default function AddInventory() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Optional: Refresh list after successful submission
        setFormData({ name: "", category: "EQUIPMENT", location: "", price: "", description: "" });
        setImages([]);
        setPreviews([]);
        const updatedRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/inventory/all`);
  ...
  const deleteAsset = async (id: number) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/inventory/delete/${id}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  return (
    <div className="min-h-screen bg-[#F0F4EF] pt-24 md:pt-32 pb-20 text-[#0D1821]">
      ...
      <div className="container mx-auto px-6 max-w-5xl">
        ...
        {isLoading ? (
          <div className="text-center p-12 opacity-30 font-heading uppercase animate-pulse">Loading Assets...</div>
        ) : inventory.length === 0 ? (
          <div className="text-center p-12 border-2 border-dashed border-[#0D1821]/10 rounded-lg">
            <h3 className="font-heading text-2xl uppercase opacity-40">No assets listed</h3>
            <p className="font-sans text-xs uppercase opacity-30 mt-2">Publish an asset to see it here.</p>
          </div>
        ) : (
